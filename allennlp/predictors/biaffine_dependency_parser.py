...
class BiaffineDependencyParserPredictor(Predictor):
    ...
    def __init__(
        self, model: Model, dataset_reader: DatasetReader, language: str = "en_core_web_lg"
    ) -> None:
        super().__init__(model, dataset_reader)
        # TODO(Mark) Make the language configurable and based on a model attribute.
        self._tokenizer = SpacyWordSplitter(language=language, pos_tags=True)

    def predict(self, sentence: str, prob=False) -> JsonDict:
        """
    ...
    @overrides
    def predict_instance(self, instance: Instance, prob=False, relation=False) -> JsonDict:
        Model.pred_prob = prob
        Model.relation_prob = relation
